...
const TAG_TERRAIN_CLASS = new Map([
  ...
  [WORLD_HEX_TAGS.GLACIER, WORLD_HEX_TERRAIN_CLASSES.OPEN_FLAT],
...
const TAG_BIOMES = new Map([
  ...
  [WORLD_HEX_TAGS.GLACIER, [WORLD_HEX_BIOMES.ARCTIC]],
...
export function buildWorldHexTagIndex({
  ...
  const fallbackTerrainTag = frozenWorld ? WORLD_HEX_TAGS.FROZEN_LANDS : WORLD_HEX_TAGS.CONTINENT;
  for (const [key, entry] of entries.entries()) {
    if (!entry?.terrainTags?.size) {
      addTag(entries, key, fallbackTerrainTag);
    }
  }
