      ...
      <div v-if="generatedSector" class="sector-results">
        <div class="sector-header">
          ...
          <div class="sector-stats">
            ...
            <span class="divider">|</span>
            <span class="stat">Realism {{ occupancyRealismLabel }}</span>
          </div>
        </div>
...
async function generateSector() {
  ...
  // When a sector is already loaded:
  //   - Preserve hexes that already have a starType (from a prior Generate Sector pass).
  //   - For hexes marked present but lacking a starType (created by "Generate All Sectors"),
  //     force system presence and assign a star type now.
  //   - Roll new stars normally for truly empty hexes.
  const existingSectorId = generatedSector.value?.sectorId ?? null;
  const existingHexMap = new Map(); // fully-assigned hexes to preserve unchanged
  const presenceOnlyCoords = new Set(); // known-occupied hexes that still need a star type
  if (existingSectorId) {
    for (const hex of generatedSector.value?.hexes ?? []) {
      if (hex.hasSystem && hex.starType) {
        existingHexMap.set(hex.coord, hex);
      } else if (hex.hasSystem) {
        presenceOnlyCoords.add(hex.coord);
      }
    }
  }
  ...
  try {
    ...
    for (let c = 1; c <= cols; c++) {
      for (let r = 1; r <= rows; r++) {
        ...
        // Preserve hexes that already have star type details assigned.
        if (existingSectorId && existingHexMap.has(coord)) {
          hexes.push({ ...existingHexMap.get(coord) });
          systemCount++;
          continue;
        }

        const isCenterHex = isGalacticCenterSector && coord === centerCoord;
        // presenceOnlyCoords: known-occupied from "Generate All Sectors" — force a system.
        const knownPresence = presenceOnlyCoords.has(coord);
        ...
        const hasSystem = isCenterHex || knownPresence || Math.random() < occupancyRate;
        if (hasSystem) {
          ...
          hexes.push({
            coord,
            hasSystem: true,
            // Not presenceOnly — a star type has just been assigned so the hex shows as ★.
            // presenceOnly:true is reserved for raw presence markers that have no star type yet.
    ...
    if (existingSectorId) {
      const upgradedCount = hexes.filter((h) => h.hasSystem && presenceOnlyCoords.has(h.coord)).length;
      const newCount = hexes.filter(
        (h) => h.hasSystem && !existingHexMap.has(h.coord) && !presenceOnlyCoords.has(h.coord),
      ).length;
      let message;
      if (upgradedCount > 0 && newCount > 0) {
        message = `Sector updated — ${upgradedCount} presence marker${upgradedCount !== 1 ? "s" : ""} assigned star types, ${newCount} new system${newCount !== 1 ? "s" : ""} discovered.`;
      } else if (upgradedCount > 0) {
        message = `Sector updated — ${upgradedCount} presence marker${upgradedCount !== 1 ? "s" : ""} assigned star types.`;
      } else if (newCount > 0) {
        message = `Sector updated — ${newCount} new stellar object${newCount !== 1 ? "s" : ""} discovered in previously empty hexes.`;
      } else {
        message = `Sector updated — no new stellar objects discovered.`;
      }
